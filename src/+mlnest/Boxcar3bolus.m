classdef Boxcar3bolus < handle & mlnest.Boxcar
    %% line1
    %  line2
    %  
    %  Created 27-Dec-2023 23:08:27 by jjlee in repository /Users/jjlee/MATLAB-Drive/mlnest/src/+mlnest.
    ...
    methods
        function this = Boxcar3bolus(varargin)
    ...
    methods (Static)
        function p = prior(~)
            ...
            p(1,:) = {'k1', 'uniform', eps, 10, 'fixed'}; % \alpha-1 for HO, OO
            p(2,:) = {'k2', 'jeffreys', 0.1, 10, 'fixed'}; % 1/\beta for HO, OO
            p(3,:) = {'k3', 'uniform', 0.5, 3, 'fixed'}; % p
            p(4,:) = {'k4', 'uniform', -2, 0, 'fixed'}; % \delta p
            p(5,:) = {'k5', 'jeffreys', 5, 30, 'fixed'}; % t_0
            p(6,:) = {'k6', 'uniform', 0, 0.2, 'fixed'}; % frac. ss
            p(7,:) = {'k7', 'uniform', 0.01, 0.2, 'fixed'}; % frac. recirc.
            p(8,:) = {'k8', 'jeffreys', 1, 60, 'fixed'}; % delay recirc.
            p(9,:) = {'k9', 'uniform', 1, 2, 'fixed'}; % amplitude
            p(10,:) = {'k10', 'jeffreys', 5, 60, 'fixed'}; % 1/\gamma
        end
        function [signal,ideal] = signalmodel(times_sampled, parnames, parvals)
            ...
            A = ks(9);
            qs_ = A*mlnest.Boxcar3bolus.solution(ks, Data); % \in [0,1], 1 Hz sampling
